...
require("reflect-metadata"); // 👈 CRITICAL: Needed to read relations
class QueryBuilder {
    ...
    with(relationName) {
        ...
        if (!rel)
            throw new Error(`Relation '${relationName}' not found on ${this._tableName}`);
        ...
        Object.entries(conditions).forEach(([key, value]) => {
            ...
            if (typeof value === "object" && value !== null && !Array.isArray(value)) {
                ...
                Object.entries(ops).forEach(([op, val]) => {
                    ...
                    const placeholder = "??";
                    if (op === "gt")
                        this._conditions.push(`${colName} > ${placeholder}`);
                    else if (op === "lt")
                        this._conditions.push(`${colName} < ${placeholder}`);
                    else if (op === "gte")
                        this._conditions.push(`${colName} >= ${placeholder}`);
                        ...
                        this._conditions.push(`${colName} LIKE ${placeholder}`);
                });
            }
