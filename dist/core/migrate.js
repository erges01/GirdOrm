...
class Migrator {
    ...
    async sync() {
        ...
        if (!models || models.length === 0) {
            console.warn("⚠️  No models registered. Did you call db.register([...models])?");
            return;
        }
        for (const model of models) {
            ...
            if (columns.length === 0) {
                console.warn(`⚠️  Skipping "${tableName}": No @Column definitions found.`);
                continue;
            }
            // ------------------------------------------------------------------
            ...
            // ------------------------------------------------------------------
            let existingColNames = [];
            try {
                const rows = await this.db.adapter.query(`SELECT column_name
           ...
           AND    table_schema = 'public';`, [tableName]);
                ...
                console.error(`  ⚠️  Could not read existing columns for "${tableName}": ${err.message}`);
            ...
            for (const col of columns) {
                ...
                try {
                    ...
                    console.log(`  ➕ Added column:  "${tableName}"."${col.name}" (${pgType})`);
                    addedAny = true;
                }
                catch (err) {
